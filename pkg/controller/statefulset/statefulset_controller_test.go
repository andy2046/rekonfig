...
// TestStatefulsetController runs Reconcile() against a fake client.
func TestStatefulsetController(t *testing.T) {
	...
	deploymentObject := testutil.ExampleStatefulSet.DeepCopy()
	...
	// Create a ReconcileStatefulSet object with the scheme and fake client.
	r := &ReconcileStatefulSet{handler: h, scheme: s}
